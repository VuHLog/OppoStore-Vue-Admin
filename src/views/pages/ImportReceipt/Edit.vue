...
onMounted(async () => {
  ...
  await proxy.$api
    ...
    .then((res) => {
      ...
      importReceipt.value.importReceiptDetails = importReceiptDetails.value.map(
        (value) => ({
          id: value.id,
...
async function updateImportReceipt() {
  errorMsg.value = "";
  ...
  await proxy.$api
    .put("/api/import-receipts/" + importReceiptId, data)
    .then((res) => {
      if (res.message) {
...
function changeImportReceiptDetailInTable(price, quantity){
  importReceiptDetail.value.price = price;
  importReceiptDetail.value.quantity = quantity;
  if(isValidImportReceiptDetail()){
    errorMsgImportReceiptDetail.value = "";
  }
}
  ...
  <div class="card mt-4">
    ...
    <div class="card-body">
      ...
      <form
        ...
        method="PUT"
        @submit.prevent="updateImportReceipt"
      >
        <div class="row">
          ...
          <div class="d-flex flex-wrap col-md-12 mb-4">
            ...
            <div
              class="col-md-1 mb-3 d-flex align-center justify-center cursor-pointer"
            >
              ...
              <table class="table table-striped">
                ...
                <tbody>
                  <template v-for="ird in importReceipt.importReceiptDetails">
                    <tr @click="importReceiptDetail = JSON.parse(JSON.stringify(ird));">
                      <td
                      ...
                      <td class="align-middle text-start text-sm">
                        <p class="text-xs text-body-table mb-0 text-start">
                          <input
                            ...
                            type="text"
                            placeholder="Giá"
                            @keyup="changeImportReceiptDetailInTable(ird.price, ird.quantity)"
                          />
                        </p>
                      </td>
                      <td class="align-middle text-start text-sm">
